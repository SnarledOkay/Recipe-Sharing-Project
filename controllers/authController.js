...
const login = async (req,res) => {
    ...
    const refreshToken = ''
    const tokenUser = UtilityFunction.createTokenUser(user)
    ...
    if(existingToken){
        ...
        UtilityFunction.attachCookiesToResponse({res,tokenUser,refreshToken})
    ...
    UtilityFunction.attachCookiesToResponse({res,tokenUser,refreshToken})
    res.status(StatusCodes.OK).json({msg:`Welcome, user ${user.name}`})
}
